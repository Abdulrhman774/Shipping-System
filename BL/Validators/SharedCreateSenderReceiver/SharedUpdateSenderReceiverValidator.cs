...
namespace BL.Validators.SharedCreateSenderReceiver
{
   public abstract class SharedUpdateSenderReceiverValidator<T>
    : AbstractValidator<T>
    where T : SharedUpdateSenderReceiverDto
    {
        protected SharedUpdateSenderReceiverValidator()
        {
            RuleFor(x => x.Name).NotEmpty().WithMessage("Name is required.")
            .MaximumLength(200).WithMessage("Name must not exceed 200 characters.")
            .Matches(@"^[a-zA-Z\u0600-\u06FF\s]+$").WithMessage("Name must contain only letters and spaces.");
